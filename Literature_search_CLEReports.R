...
#------set directory
here <- setwd("C:/Users/prist/Documents/Repos_EHA/CurrentLandscape/Naive")
...
cutoff_fig <- ggplot(term_strengths, aes(x=rank, y=strength, label=term)) +
  geom_line() +
  geom_point() +
  geom_text(data=dplyr::filter(term_strengths, rank>40), hjust="right", nudge_y=30, check_overlap=TRUE)
cutoff_fig

#----retaining 80% of the search terms
cutoff_cum <- litsearchr::find_cutoff(g, method="cumulative", percent=0.7)
cutoff_cum
...
#----choosing the 2th cut (to keep the largest amount of words) 
g_redux <- litsearchr::reduce_graph(g, cutoff_change[2])
selected_terms <- litsearchr::get_keywords(g_redux)
selected_terms
length(selected_terms)

#---adding extra terms that were not selected
extra_terms <- c("neotropics","forest")
...
grouped_terms <-list(neotropics=final_terms[c(5,6,12,15,16)],
                     diseases=final_terms[c(1,2,3,4,7,8,10,13,14)])
...
"((\"tropical disease\" OR \"tropical region\" OR neotropics OR forest) AND 
(\"infectious disease\" OR \"public health\" OR \"risk factor\" OR \
"zoonotic disease\" OR \"borne disease\" OR \"neglected tropical\" OR \
"zoonotic pathogen\"))"
